...
- Fecha de Última Edición / Last Edition Date:
     05 de febrero de 2024 / February 05th, 2024
...
% INGRESO DE INFORMACIÓN / INFORMATION INPUT:
     ...
     switch(Oper)
         ...
         case "Desorption"
         % Required to Input:
         L1 = 110; L2 = 101; LS = 100; Li1 = 10; Li2 = 1; % Flujos de Líquido / Liquid Flows 
         Y1i = 0.001; % Concentración Inicial de Flujo de Gas / Gas Flow Initial Concentration
         aFlujMin = 1.3; % Factor de Operación para Flujo de Gas Mínimo / Operation Factor for Minimum Gas Flow
         % Automatic Calculation:
...
switch(Oper)
     ...
     % Information Output:
          ...
          subplot(1, 2, 2); grid on; hold on; axis([X2i X1i Y2i Y1i]);
          xlabel("Concentración en Fase Líquida (X)"); ylabel("Concentración en Fase de Vapor (Y)");
...
case("Desorption")
     ...
     while Crit == 0
          LinFlujMinPos = @(X) MFlujMinPos * X + (Y1i - MFlujMinPos * X2i);
          LinFlujMinNeg = @(X) MFlujMinNeg * X + (Y1i - MFlujMinNeg * X2i);
          YFlujMinPos = LinFlujMinPos(XFlujMin); YFlujMinNeg = LinFlujMinNeg(XFlujMin);

          for i = 1 : length(XEQ)
               if YFlujMinPos(i) < YEQ(i) && min(abs(YFlujMinPos - YEQ)) < Tol 
                    CritP(i) = 1;
                    MFlujMin = MFlujMinPos;
                    %disp("Por arriba");
               else
                    CritP(i) = 0;
          ...
          CritNaux = CritN; CritPaux = CritP;
          CritP = linspace(0, 0, length(XEQ)); CritN = CritP;
          k = k + 1;

     end
               
     LinFlujMin = @(X) MFlujMin * X + (Y1i - MFlujMin * X2i);
     YFlujMin = LinFlujMin(XFlujMin);

     FlujMin = LS / MFlujMin;

     % Information Output:
     plot(XEQ, YEQ); hold on; grid on; hold on;
     %plot(DXEQ, DYEQ); hold on;
     plot(XFlujMin, YFlujMin); hold on;
